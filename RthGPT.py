...
def generator(context_size=CONTEXT_SIZE):  # Default context window size to 2048
    ...
    for features in dataset["train"]:
        ...
        for ma_token in ma:
            ...
            regulated_speaker_context = tensorize(lenreg(speaker_context, size=context_size, padding_idx=0),
                                                  dtype=tf.int64)
...
with tf.device('/CPU:0'):
    model.fit(dataset, epochs=100, steps_per_epoch=2048, verbose=1, batch_size=4,
              callbacks=[tensorboard_callback, nan_stop, checkpoint_callback])
    model.save("test")
